    ...
    foreach ($Routes as $routeItem) {
        ...
        if (preg_match_all("/\{(\w+)\}/i", $route_match, $match)) {
            ...
            if (preg_match($route_match, $route, $match_value)) {
                ...
                return array($routeItem['controller'], $params, (isset($routeItem['permission']) ? $routeItem['permission'] : ''));
            }
        } else if (strcasecmp($route_match, $route) == 0) {
            return array($routeItem['controller'], array(), (isset($routeItem['permission']) ? $routeItem['permission'] : ''));
        }
    }
}
