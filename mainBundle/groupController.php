...
class groupController
{
    ...
    public function deleteFirmarePriceAction($twig, $params)
    {
        if ($this->getRole() == 1 or $this->getRole() == 2 or $this->getRole() == 3) {
            $id = intval($_POST['id']);
            $query = mysqli_query($this->dbConnect, "delete from groups where id = {$id}");
    ...
    public function clonePriceAction($twig, $params)
    {
        if ($this->getRole() == 1 or $this->getRole() == 2 or $this->getRole() == 3) {
            $id = intval($_POST['groupId']);
            $query = mysqli_query($this->dbConnect, "select max(id) as id from groups");
            $result = mysqli_fetch_assoc($query);
            $maxId = $result['id'] + 1;

            $query = mysqli_query($this->dbConnect, "select * from groups where id = {$id}");
            $result = mysqli_fetch_assoc($query);

            $name = $result['name'] . "_" . $maxId;
            mysqli_query($this->dbConnect, "insert into groups set name = '{$name}', parrent = '{$this->user}'");
            $rezz = mysqli_query($this->dbConnect, "SELECT last_insert_id()");
            $lastId = mysqli_fetch_row($rezz);

            $query = mysqli_query($this->dbConnect, "select * from prices where parrent_group = {$result['id']}");
            while ($result = mysqli_fetch_assoc($query)) {
                mysqli_query($this->dbConnect, "insert into prices set fid = '{$result['fid']}', price = '{$result['price']}', enabled = '{$result['enabled']}', parrent_group = '{$lastId[0]}'");
            }
    ...
    public function showPriceAction($twig, $params)
    {
        if ($this->getRole() == 1 or $this->getRole() == 2 or $this->getRole() == 3) {
            $group_id = intval($_POST['id']);
            $q = mysqli_query(
                $this->dbConnect,
                "select p.id as pid,v.name as vendor, m.name as model, f.name as firmware, f.type, p.price, p.enabled, f.description from prices p, firmware f, models m, vendors v where p.fid = f.id and f.model_id = m.id and m.vendor_id = v.id and parrent_group = '{$group_id}' order by v.name,m.name"
            );
            $price = array();
            ...
            while ($r = mysqli_fetch_assoc($q)) {
                ...
                $display .= '<tr id="firmware_' . $r['pid'] . '">
                    <td width="100px">' . $r['vendor'] . '</td>
                    <td width="100px">' . $r['model'] . '</td>
                    <td width="100px">' . $r['firmware'] . '</td>
                    <td width="100px"><input class="change_price" data-id="' . $r['pid'] . '" value="' . $r['price'] . '"></td>
                    <td width="20px"><input class="change_enabled" type="checkbox" data-id="' . $r['pid'] . '" value="1" ' . $check . '></td>
                </tr>';
